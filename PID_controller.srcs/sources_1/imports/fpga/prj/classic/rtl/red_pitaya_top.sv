...
assign dac_a_sum = pid_dat[0];
assign dac_b_sum = pid_dat[1];
...
/////////////////////////////
// Switches to turn on/off Kp Ki individually 
/////////////////////////////
logic KpEnabled_buf_o;
logic KpEnabled_buf_i;

logic KiEnabled_buf_o;
logic KiEnabled_buf_i;


//assign  switch_test = switch_buf_o;
logic [7:0] led_buf_i;
logic [7:0] led_buf_o;
assign led_buf_i[7] = KpEnabled_buf_o;
assign led_buf_i[6] = KiEnabled_buf_o;

//IBUF ibuf_switch (.I(exp_p_io[6]), .O(switch_buf_in));
IOBUF ibuf_KpEnabled  (.O(KpEnabled_buf_o), .IO(exp_p_io[6]), .I(KpEnabled_buf_i), .T(1'b1) );
IOBUF ibuf_KiEnabled  (.O(KiEnabled_buf_o), .IO(exp_n_io[6]), .I(KiEnabled_buf_i), .T(1'b1) );

IOBUF iobuf_led   [8-1:0] (.O(led_buf_o), .IO(led_o),    .I(led_buf_i), .T(8'b0) );
...
red_pitaya_pid i_pid (
  ...
  .KpEnabled    (   KpEnabled_buf_o ),
  .KiEnabled    (   KiEnabled_buf_o ),
  // System bus
  .sys_addr        (  sys_addr      ),  // address
  .sys_wdata       (  sys_wdata     ),  // write data
  .sys_sel         (  sys_sel       ),  // write byte select
